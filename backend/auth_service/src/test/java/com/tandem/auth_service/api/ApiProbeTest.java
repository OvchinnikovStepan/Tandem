...
class ApiProbeTest {
    ...
    void probe_all_endpoints() {
        ...
        var regPhone = BASE.send("POST", "/register/phone",
                Map.of("phoneNumber", "+12345678901"), null);
        System.out.printf("POST /register/phone -> %d | %s%n%n", regPhone.statusCode(), regPhone.body());

        var regVerify = BASE.send("POST", "/register/verify",
                Map.of("verificationId", "test-id-123", "code", "123456"), null);
        System.out.printf("POST /register/verify -> %d | %s%n%n", regVerify.statusCode(), regVerify.body());

        var regEmail = BASE.send("POST", "/register/email",
                Map.of("verificationId", "test-id-123", "email", "probe@test.com", "password", "Qwerty123"), null);
        System.out.printf("POST /register/email -> %d | %s%n%n", regEmail.statusCode(), regEmail.body());

        var login = BASE.send("POST", "/login",
                Map.of("email", "probe@test.com", "password", "Qwerty123"), null);
        System.out.printf("POST /login -> %d | %s%n%n", login.statusCode(), login.body());

        var login2 = BASE.send("POST", "/login",
                Map.of("email", "user@example.com", "password", "password123"), null);
        System.out.printf("POST /login (example) -> %d | %s%n%n", login2.statusCode(), login2.body());

        var refresh = BASE.send("POST", "/refresh",
                Map.of("refreshToken", "dummy-token"), null);
        System.out.printf("POST /refresh -> %d | %s%n%n", refresh.statusCode(), refresh.body());

        var meNoToken = BASE.send("GET", "/me", null, null);
        System.out.printf("GET /me (no token) -> %d | %s%n%n", meNoToken.statusCode(), meNoToken.body());

        var meFake = BASE.send("GET", "/me", null, "fake-bearer-token");
        System.out.printf("GET /me (fake token) -> %d | %s%n%n", meFake.statusCode(), meFake.body());

        var logout = BASE.send("POST", "/logout", null, "fake-bearer-token");
        System.out.printf("POST /logout (fake token) -> %d | %s%n%n", logout.statusCode(), logout.body());

        var pwdStrength = BASE.send("POST", "/password/check-strength",
                Map.of("password", "Qwerty123"), null);
        System.out.printf("POST /password/check-strength -> %d | %s%n%n", pwdStrength.statusCode(), pwdStrength.body());

        var resetReq = BASE.send("POST", "/password/reset/request",
                Map.of("identifier", "probe@test.com", "method", "email"), null);
        System.out.printf("POST /password/reset/request -> %d | %s%n%n", resetReq.statusCode(), resetReq.body());

        var resetVerify = BASE.send("POST", "/password/reset/verify",
                Map.of("resetToken", "dummy-reset-token"), null);
        System.out.printf("POST /password/reset/verify -> %d | %s%n%n", resetVerify.statusCode(), resetVerify.body());

        var resetComplete = BASE.send("POST", "/password/reset/complete",
                Map.of("resetToken", "dummy-reset-token", "newPassword", "NewPass123"), null);
        System.out.printf("POST /password/reset/complete -> %d | %s%n%n", resetComplete.statusCode(), resetComplete.body());

        var sessions = BASE.send("GET", "/sessions", null, "fake-bearer-token");
        ...
        if (!verificationId.isBlank()) {
            ...
            for (String code : codes) {
                var freshReg = BASE.send("POST", "/register/phone",
                        Map.of("phoneNumber", "+1234567" + code.substring(0, 4)), null);
                String freshVid = freshReg.body().path("verificationId").asText("");

                var verify = BASE.send("POST", "/register/verify",
                        Map.of("verificationId", freshVid, "code", code), null);
                ...
                if (verify.statusCode() == 200) {
                    ...
                    var emailReg = BASE.send("POST", "/register/email",
                            Map.of("verificationId", freshVid, "email", "fullflow@test.com", "password", "Qwerty123"), null);
                    System.out.printf("POST /register/email -> %d | %s%n%n", emailReg.statusCode(), emailReg.body());

                    var loginAfterReg = BASE.send("POST", "/login",
                            Map.of("email", "fullflow@test.com", "password", "Qwerty123"), null);
                    ...
                    if (!at.isBlank()) {
                        var meReal = BASE.send("GET", "/me", null, at);
                        ...
                        if (!rt.isBlank()) {
                            var refreshReal = BASE.send("POST", "/refresh",
                                    Map.of("refreshToken", rt), null);
                            System.out.printf("POST /refresh (real) -> %d | %s%n%n", refreshReal.statusCode(), refreshReal.body());
                        }

                        var logoutReal = BASE.send("POST", "/logout", null, at);
