...
function PostCard({ post }: { post: Post }) {
    return (
        <div className="relative bg-icon-gray overflow-hidden cursor-pointer group aspect-square rounded-xl">
            {post.imageUrl && (
                <img
                    src={post.imageUrl}
                    alt=""
        ...
        return (
            <div className="grid grid-cols-3 gap-1 w-full">
                {Array.from({ length: 9 }).map((_, index) => (
                    <div
                        key={index}
                        className="aspect-square bg-icon-gray rounded-xl"
                    />
                ))}
            </div>
        );
        ...
        return (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400 w-full">
                <p className="text-lg font-roboto">
                    {t("profile.posts.emptyTitle")}
                </p>
                <p className="text-sm mt-1 font-roboto">
                    {t("profile.posts.emptyDescription")}
                </p>
            </div>
        );
    }
